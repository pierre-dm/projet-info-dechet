...
public class HO2theme1 {
    ...
    private static void menuProblematique1(Scanner sc) {
        while (true) {
            ...
            try {
                graph g = new graph();
                // graphe orienté de base (cas 1 de la problématique 2 aussi)
                g.chargerDepuisFichier("graph1HO2.txt");
    ...
    public static void pb1hypothese1(graph g, Scanner sc) {
        ...
        Integer u = g.getId(nomU);
        Integer v = g.getId(nomV);
        if (u == null || v == null) {
        ...
        // Dijkstra "aller" depuis CT sur le graphe orienté
        Resultat aller = dijkstra.executer(g.getAdj(), idCT);
        // Dijkstra "retour" depuis CT sur le graphe inversé
        ...
        // -------- Cas 1 : seulement U->V (sens unique) --------
        if (wUV >= 0 && wVU < 0) {
            if (aller.dist[u] == INF || retour.dist[v] == INF) {
                System.out.println("U ou V n’est pas atteignable depuis CT.\n");
                return;
            }
            ...
            // U -> V
            chemin.add(v);
            // V -> ... -> CT (via chemin dans le graphe inversé)
            List<Integer> cheminVversCT = retour.reconstruireChemin(v); // CT -> ... -> V dans G^(-1)
            Collections.reverse(cheminVversCT); // V -> ... -> CT dans G
            for (int i = 1; i < cheminVversCT.size(); i++) chemin.add(cheminVversCT.get(i));
        ...
        // -------- Cas 2 : double sens (U->V et V->U) --------
        if (wUV >= 0 && wVU >= 0) {
            if (aller.dist[u] == INF || aller.dist[v] == INF ||
                    retour.dist[u] == INF || retour.dist[v] == INF) {
                System.out.println("U ou V n’est pas atteignable depuis CT.\n");
                return;
            }
            ...
            // Pour chaque cas valide, on reconstruit le chemin et on garde le meilleur
            // 1)
            {
    ...
    public static void pb1hypothese2(graph g, Scanner sc) {
        ...
        // Liste des particuliers : on stocke les deux extrémités U et V de l'arc U->V
        ...
        for (int i = 1; i <= k; i++) {
            ...
            // On suppose le particulier sur U->V, mais si seul V->U existe on échange
            int wUV = getDistanceOriented(g, idU, idV);
            int wVU = getDistanceOriented(g, idV, idU);

            if (wUV < 0 && wVU < 0) {
                System.out.println("Erreur : il n'existe pas d'arête entre "
                        + nomU + " et " + nomV + ".\n");
                return;
            }

            // Si seul V->U existe, on renomme pour respecter la convention "particulier sur U->V"
            if (wUV < 0 && wVU >= 0) {
                int tmp = idU; idU = idV; idV = tmp;
            }

            particuliers.add(new Particulier(idU, idV));
        }
        ...
        // Heuristique du plus proche "particulier"
        while (restant > 0) {

            // Plus court chemin depuis le sommet courant A
            Resultat resFromA = dijkstra.executer(g.getAdj(), current);

            int bestIdx  = -1;
            int bestCost = INF;
            // Pour reconstruire le meilleur déplacement
            List<Integer> bestSegment = null;
            int bestNewCurrent = -1;   // nouveau sommet de départ après ce particulier
            ...
            for (int i = 0; i < particuliers.size(); i++) {
                ...
                int wUV = getDistanceOriented(g, u, v);   // doit exister
                int wVU = getDistanceOriented(g, v, u);   // peut ne pas exister

                if (wUV < 0) continue; // sécurité : on veut forcément U->V

                // Cas double sens : comparer
                // 1) A -> ... -> U -> V
                // 2) A -> ... -> V -> U -> V
                // Si pas de V->U, seule la 1) est possible.
                ...
                // Candidat 2 : A..V->U->V (possible seulement si V->U existe)
                if (wVU >= 0 && dAv < INF) {
            ...
            // On ajoute le segment choisi au tour global
            // (en évitant de répéter le sommet de départ déjà présent)
    ...
    public static void problematique2(Scanner sc) {
        ...
        switch (choix) {
            case "1":
                // même graphe orienté que pour la problématique 1
    ...
    // Distance d'un arc ORIENTÉ (from -> to), ou -1 s'il n'existe pas
    private static int getDistanceOriented(graph g, int from, int to) {
        for (rue r : g.getAdj().get(from)) {
            if (r.getDestination() == to) {
    ...
    private static List<List<rue>> construireAdjInverse(graph g) {
        ...
        for (int u = 0; u < n; u++) {
            for (rue r : adj.get(u)) {
                ...
                int d = r.getDistance();
                inv.get(v).add(new rue(u, d)); // arc v -> u dans le graphe inverse
            }
        }
        return inv;
    }
